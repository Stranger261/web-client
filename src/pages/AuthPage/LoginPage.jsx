...
const LoginPage = () => {
  ...
  const loginHandler = async e => {
    ...
    try {
      const apiCall = login(formData);
      const delayTime = new Promise(res => setTimeout(res, 1000));

      const [apiCallResult] = await Promise.all([apiCall, delayTime]);

      toast.success(apiCallResult.data.message);

      navigate(
        `/${apiCallResult.data.data.safeUser.role}/${
          apiCallResult.data.data.safeUser.role === 'patient'
            ? 'my-dashboard'
            : 'dashboard'
        }`
      );
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center px-6 py-12 lg:px-8">
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md bg-white p-8 rounded-3xl shadow-lg">
        ...
        <p className="mt-6 text-center text-sm">
          <Link
            to="/"
            className="font-medium text-gray-600 hover:text-[#172554]"
          >
            Back to Home
          </Link>
        </p>
