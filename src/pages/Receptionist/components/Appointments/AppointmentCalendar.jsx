  ...
  const fetchAppointments = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:56741/api/v1/appointments/slots/${formattedDate}?${queryParams}`,
      );
      ...
      if (result.success) {
        setAppointments(result.data.slots.res || []);
        setPagination(result.data.slots.pagination);
...
const EmptyState = () => {
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 p-8 flex flex-col items-center justify-center h-full min-h-[400px] sm:min-h-[600px]">
      <div className="w-16 h-16 sm:w-20 sm:h-20 bg-blue-50 dark:bg-blue-900/30 rounded-full flex items-center justify-center mb-4">
        <CalendarIcon className="w-8 h-8 sm:w-10 sm:h-10 text-blue-600 dark:text-blue-400" />
      </div>
      ...
      <p className="text-sm sm:text-base text-gray-500 dark:text-gray-400 text-center max-w-md px-4">
        Choose a date from the calendar to view appointments and available slots
      </p>
    </div>
  );
};
      ...
      <div className="px-4 sm:px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-gray-800 dark:to-gray-800">
        <div className="flex items-start sm:items-center justify-between gap-4">
          ...
          <button
            ...
            className="px-3 sm:px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors flex items-center gap-2 text-sm sm:text-base flex-shrink-0"
          >
          ...
          {appointments.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 px-4">
              ...
              <p className="text-sm sm:text-base text-gray-500 dark:text-gray-400 text-center max-w-md">
                There are no appointments scheduled for this date with the
                current filters.
              </p>
            </div>
          ) : (
